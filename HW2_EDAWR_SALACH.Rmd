...
```{r}
library(ggplot2)
# using this to convert axis from scientific notation to something I can more easily interpret
library(scales)
```
...
```{r read_countydata}
# a simple read.csv seems to work fine for now
county_df <- read.csv("~/Documents/HW2/hw2_files/data/county.csv")
county_mi_df <- read.csv("~/Documents/HW2/hw2_files/data/county_MI.csv")
head(county_df)
tail(county_df)
head(county_mi_df)
```
...
> County census information taken from 2015 ACS survey.
...
> I only see one NA under child poverty.  So not a big issue with this data.
...
```{r summary_stats}
# median income for US.  
ggplot(data = county_df) + 
  geom_histogram(aes(x = Income), binwidth = 2500) + 
  # lose the scientific notation; 
  # I stil don't think the axis is quite correct.  Wanted it to start at 0
  scale_x_continuous(breaks = seq(0, 200000, 20000), labels = comma)
  
# median income for US; kernel density plot. 
# need to learn what this plot does
ggplot(data = county_df) + 
...
# income per cap for US counties.  Copy and paste from above.  
ggplot(data = county_df) + 
  geom_histogram(aes(x = IncomePerCap), binwidth = 2500) + 
  # lose the scientific notation; 
  # I stil don't think the axis is quite correct.  Wanted it to start at 0
  scale_x_continuous(breaks = seq(0, 200000, 20000), labels = comma)

# income per capita for US; kernel density plot.  
ggplot(data = county_df) + 
...
# check minimum and max county income since my histo looked off.
print("Lowest county median income:")
min(county_df$Income)
print("Highest county median income:")
max(county_df$Income)

print("Lowest county income per cap:")
min(county_df$IncomePerCap)
print("Highest county income per cap:")
max(county_df$IncomePerCap)

print('Range of income per cap:')
max(county_df$IncomePerCap) - min(county_df$IncomePerCap)
print('Range of median income:')
max(county_df$Income) - min(county_df$Income)

print("Mean county income per cap:")
mean(county_df$IncomePerCap)
# hmmmmm.....strange way to look at this
print("Mean county median income:")
mean(county_df$Income)

print("Median county median income:")
median(county_df$Income)
print("Median county income per capita:")
median(county_df$IncomePerCap)

print("Standard Dev median income:")
sd(county_df$Income)
print("Standard dev county median income:")
sd(county_df$Income)

# percentiles using a vector.  This is cool.
print("95th and 99th percentile median income:")
quantile(county_df$Income, c(.95, .99))
print("95th and 99th percentile income per capita:")
quantile(county_df$IncomePerCap, c(.95, .99))

# interquartile range using a vector.  This is cool.
print("25th and 75th percentile median income:")
quantile(county_df$Income, c(.25, .75))
print("25th and 75th percentile income per capita:")
quantile(county_df$IncomePerCap, c(.25, .75))

# Coefficient of variation.  
print("Coeff of variation for Income:")
mean(county_df$Income) / sd(county_df$Income)
print("Coeff of variation for income per capita:")
mean(county_df$IncomePerCap) / sd(county_df$IncomePerCap)
...
> Both median income and Income per Capita are a little bit right skewed, but otherwise look a bit normal-ish.  There are outliers for high income counties that skew the data to the right.  Lop off the outliers and you could convince me it was a bell curve.
>
...
# median income for US.  
ggplot(data = county_df) + 
  geom_histogram(aes(x = Income), fill = 'blue', color = 'white', binwidth = 2500) + 
  ylab('Number of Counties') +
  ggtitle("Distibution of Median Income by County") + 
  # lose the scientific notation; 
  scale_x_continuous(breaks = seq(0, 200000, 20000), labels = comma, 'Median Income')
...
ggplot(county_df) + 
  geom_boxplot(aes(Region, Income))

ggplot(county_df) + 
  geom_violin(aes(Region, Income))

```

> It's easy to see from skewness in the data from these plots.  The box plot shows plenty of outliers in the form of the dots, or points, in the high income range.  The violin plots help us see the same pattern in the data, with the added benefit that we can see where most of the income is concentrated.  In the Midwest, the violin shap is "fat" which shows us that many counties have a median income level around $50,000.  The West region, has a slimmer shape.  There may be more variability in the median income for the west region.  We can confirm with a standard deviation by region.
>
...
ggplot(county_mi_df) + 
  ...
  ggtitle("Median Michigan Income by County and Peninsula") + 
  # Center this bad boy
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(labels = comma, 'Median Income by County') + 
  scale_x_discrete('Michigan Peninsula')
...
> Absolutely.  The Upper Peninsula does not have the right skewness in distribution that the Lower Peninsula seems to have.  No very wealthy counties north of the bridge.
...
ggplot(county_mi_df) +
  ...
  ggtitle("County Unemployment Rates by Peninsula") + 
  theme(plot.title = element_text(hjust = 0.5)) 
  
ggplot(county_mi_df) + 
  geom_violin(aes(x = peninsula, y = Drive)) + 
  ggtitle("Pecentage of Drivers by County") + 
  theme(plot.title = element_text(hjust = 0.5))
 
ggplot(county_mi_df) + 
  geom_boxplot(aes(peninsula, Poverty)) + 
  ggtitle("Percentage of White or Caucausion by County") + 
  # Just experimenting....
  theme(plot.title = element_text(hjust = 0.5, vjust = -80.5))

```

> The mean unemployment rates between UP and LP are fairly similar; however, the distribution of these rates skews to the right for the Upper Peninsula.  Looks like there may be some conties struggling up there.

> There appears to be fewer drivers up north.  I wonder why that is?  I thought there may be more poverty up there, but that does not appear to be the case based on the third box plot.
...
ggplot(county_df) + 
  # Audi Red
  geom_bar(aes(State), fill = rgb(red=187, green=10, blue=48, maxColorValue = 255)) + 
...
ggplot(county_df) + 
  geom_bar(aes(State, fill = Region)) + 
  coord_flip() +
  scale_y_continuous('Number of Counties') + 
  ggtitle('Number of Counties by State')
...
```{r xy_inc_commute}
ggplot(county_mi_df) + 
  geom_point(aes(IncomePerCap, MeanCommute))
...
ggplot(county_mi_df) + 
  geom_point(aes(IncomePerCap, MeanCommute, color = peninsula, size = TotalPop))
```

> Looks like they are moderately positively correlated.  People seem to be willing to drive further to work if there is money as a carrot.  Or....it might be that counties are more wealthy BECAUSE people are willing to drive to work.  Correlation <> Causality.  The wealthier counties are all in the Lower Peninsula.
...
ggplot(county_mi_df) + 
  geom_point(aes(IncomePerCap, Professional, size = TotalPop)) + 
  facet_wrap(~ peninsula)

ggplot(county_mi_df) + 
  geom_point(aes(Income, Unemployment, size = Men)) + 
  facet_wrap(~ peninsula)
```

> In both cases, the lower peninsula data has a much stronger correlation between variables than the upper peninsula.  Possibly the lower population in the UP is resulting in this?  Or maybe just fewer counties creates fewer opportunities to witness the relationship in action.
...
```{r xy_pop_inc}
ggplot(county_df) +
  geom_point(aes(TotalPop, IncomePerCap)) + 
...
ggplot(county_df) +
  geom_point(aes(log(TotalPop), IncomePerCap)) + 
    scale_x_continuous(labels = comma) + 
    scale_y_continuous(labels = comma)

# find the correlation between these two variables
cor.test(county_df$TotalPop, county_df$IncomePerCap, method = "pearson", conf.level = 0.95)
cor.test(log(county_df$TotalPop), county_df$IncomePerCap, method = "pearson", conf.level = 0.95)


```

> Taking the logarithm of Total Population here allows us to view the data based on the percent change in the data.  So many of the counties had similar populations that it makes a visualization difficult.  This allows us to see that there is in fact somewhat of a positive correlation between Income per Capita and Total Population.  Pearson's R shows us, however, that the correlation isn't that strong, at .293 for log(TotalPop) and Income per Capita.
