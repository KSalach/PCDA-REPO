...
> The below is more useful, though you still don't get the full picture of the outliers.  Credit "Joe" on Stack Exchange for the idea.  https://stackoverflow.com/questions/28198613/r-ggplot-boxplot-change-y-axis-limit/28199268
...
# strugglesville getting the x axis to read with an axis title and correct labels
ggplot(DowDelaySummary) + 
  geom_bar(aes(FL_DOW, avg_delay), stat = 'Identity') + 
...
modelfit2 <- lm(DEP_DELAY ~ FL_DOW + CRS_DEP_HOUR + DISTANCE + CARRIER, data = flights_out_train)
...
modelpredict2RMSE 
```
> Okay, so not much better.  I was receiving an issue with adding in DEST to the model.  If I had been able to resolve that, maybe I would have had some better luck.
